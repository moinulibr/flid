			...
			<div class="page-content-wrapper">
				...
				<div class="page-content">
					...
					<div class="page-breadcrumb d-none d-sm-flex align-items-center mb-3">
						...
						<a href="{{route('admin.flss.post.create')}}" class="btn btn-sm btn-outline-secondary">Add New</a>
					</div>
					<!--end breadcrumb-->
						...
						<div class="card-body">
							<ul class="nav nav-tabs" id="myTab" role="tablist">
								<li class="nav-item" role="presentation">
									<a class="nav-link {{$ptyUrl == NULL ? 'active' : ''}}" href="{{route('admin.flss.post.index','')}}">
										All <span>({{$postCountables->count()}})</span>
									</a>
								</li>
								<li class="nav-item" role="presentation">
									<a class="nav-link {{$ptyUrl == 'published' ? 'active' : ''}}" href="{{route('admin.flss.post.index','published')}}">
										Published
										<span>({{$postCountables->where('status',1)->count()}})</span>
									</a>
								</li>
								<li class="nav-item" role="presentation">
									<a class="nav-link {{$ptyUrl == 'draft' ? 'active' : ''}}" href="{{route('admin.flss.post.index','draft')}}">
										Draft  
										<span>({{$postCountables->where('status',2)->count()}})</span>
									</a>
								</li>
								<li class="nav-item" role="presentation">
									<a class="nav-link {{$ptyUrl == 'reqtopub' ? 'active' : ''}}" href="{{route('admin.flss.post.index','reqtopub')}}">
										Request To Publish  
										<span>({{$postCountables->where('status',3)->count()}})</span>
									</a>
								</li>
								{{-- <li class="nav-item" role="presentation">
									<a class="nav-link {{$ptyUrl == 'pending' ? 'active' : ''}}" href="{{route('admin.flss.post.index','pending')}}">
										Pending  
												...
												<tr>
													...
													<td>
														...
														<div class="group-link">
															<a class="#" href="{{route('admin.flss.post.edit',$item->id)}}"> Edit</a> <span class="separetor"> | </span> 
															<a class="deleteClass" data-href="{{route('admin.flss.post.delete',$item->id)}}" href="#"> Trash</a> <span class="separetor"> | </span>
															<a class="#" href="#"> View</a>
														</div>
													</td>
													<td>
       ...
       /*  $(document).on('click', '.deletedAll', function (){
            ...
            $(document).on('click', '.deletedAllButton', function (){
               ...
               if(option == 0)
               {
                ...
                var url =  "{{ route('admin.flss.post.bulk.deleting') }}";

                if(ids.length <= 0) return ;
                let decirectUrl = "{{route('admin.flss.post.index')}}";
                $.ajax({
                    url: url,
                    data: {ids: ids},
                    type: "POST",
