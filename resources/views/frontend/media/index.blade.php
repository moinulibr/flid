    ...
    <nav class="navbar navbar-light navbar-expand-md py-3">
        <div class="container">
           <a href="{{route('frontend.webview.menu.list')}}">
            ...
            <span class="page-title">মিডিয়া</span>
            </a>
        ...
        <div class="row">
            <div class="col-8">
                @php
                    ...
                    //echo $bangla_date;
                @endphp
                <h1 class="text-start title">{{$bangla_convert}}</h1>
            </div>
            <div class="col-4 text-end" style="font-family: 'SolaimanLipi';font-style: normal;font-weight: 400;font-size: 10px;margin-bottom: 0;">
                {{-- <a href="{{route('frontend.webview.view.media.date.wise.details',date('Y-m-d',strtotime($item->created_at)))}}" style="font-size: 14px;">সব গুলো দেখুন</a> --}}
                <a href="{{route('frontend.webview.view.media.date.wise.details.by.media.id',$item->id)}}" style="font-size: 14px;">সব গুলো দেখুন</a>
            </div>
        </div>
        <div class="row" style="padding-bottom: 10px;">
            {{-- @php
                $mediaDatewise = App\Models\Backend\Media::whereDate('created_at','=',date('Y-m-d',strtotime($item->created_at)))->latest()->get()->take(1);
            @endphp --}}
            <div class="col">
                <a href="#" class="image-wrapper">
                    {{-- @foreach ($mediaDatewise as $media)
                    <div style="margin: 2px;">
                        <img  src="{{ asset('storage/media/'.$media->photo) }}" class="img-fluid" alt="image">
                    </div>
                    @endforeach --}}
                    <div style="margin: 2px;">
                        <img  src="{{ asset('storage/media/'.$item->featured_image) }}" class="img-fluid" alt="image">
                    </div>
                </a>
            </div>
        </div>
